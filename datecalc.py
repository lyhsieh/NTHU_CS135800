...
if __name__ == '__main__':
    ...
    def enter(value):
        ...
        operand_entry.insert(tkinter.END, f'{value}' + ' ')
    ...
    def final_date():
        ...
        print(f'operand_entry.get() = {operand_entry.get()}')
        #tmp = str(tmp)
        #tmp = ''.join(tmp)
        #print(f'tmp = {tmp}, type of tmp = {type(tmp)}')
        print(f'tmp.split() = {tmp.split()}')
        for item in tmp.split():
            if item in {'today', 'tomorrow', 'yesterday', 'add', 'sub', 'swap'}:
                list_in.append(item)
            else:
                list_in.append(eval(item))
        print(f'list_in = {list_in}')
        final = datecalc(*list_in)
        operand_entry.delete(0, tkinter.END)
        print(f'final = {final}, type of final = {type(final)}')
